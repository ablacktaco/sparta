...
class SendingStationViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        getStationGoods { (stationGoods) in
            self.sendingStationGoods = stationGoods.result.filter { ($0.status == "運送中") }
            DispatchQueue.main.async {
                UIView.animate(withDuration: 0.3) {
                    self.loadingTable.alpha = 0
                }
                if self.sendingStationGoods.count == 0 {
                    self.sendingStationGoodsTable.backgroundView = self.noGoodsView
                }
                self.sendingStationGoodsTable.reloadData()
            }
        }
...
extension SendingStationViewController {
